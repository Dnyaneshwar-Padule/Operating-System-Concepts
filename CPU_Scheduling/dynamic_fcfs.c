...
void sort_AT()
{
    ...
    for (int i = n - 1; i > 0; i--)
    {
        for (int j = 0; j < i; j++)
        {
            if ( processes[j].arriving_time > processes[i].arriving_time )
            {
                temp = processes[i];
...
void increase_gantt_chart()
{
    g = (gantt_chart*)realloc(g, 1 + ( g_cnt * sizeof(gantt_chart) ));
}
...
void fcfs()
{
    int time = 0;
    int i = 0;
    g = (gantt_chart*)malloc(sizeof(gantt_chart) * n);
